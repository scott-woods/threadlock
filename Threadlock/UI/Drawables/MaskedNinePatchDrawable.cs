...
namespace Threadlock.UI.Drawables
{
    public class MaskedNinePatchDrawable : IDrawable
    {
        ...
        public MaskedNinePatchDrawable(NinePatchSprite sprite)
        {
            _sprite = sprite;
        ...
        /// <summary>
        /// creates a MaskedNinePatchDrawable using the full texture
        /// </summary>
        ...
        public MaskedNinePatchDrawable(Texture2D texture, int left, int right, int top, int bottom) : this(
            new NinePatchSprite(texture, left, right, top, bottom))
        { }

        public MaskedNinePatchDrawable(Sprite sprite, int left, int right, int top, int bottom) : this(
        ...
        public void Draw(Batcher batcher, float x, float y, float width, float height, Color color)
        {
            ...
            if (width < _sprite.Left)
            {
                var actualWidth = Math.Max((int)width, 5);
                var sourceRect = _sprite.NinePatchRects[3];
                var copyRect = new Rectangle(sourceRect.X, sourceRect.Y, actualWidth, sourceRect.Height);
                batcher.Draw(_sprite, new Rectangle((int)x, (int)y, actualWidth, (int)height), copyRect, color);
            }
            else if (width < _sprite.Left + _sprite.Right)
            {
                //draw left side
                batcher.Draw(_sprite, new Rectangle((int)x, (int)y, _sprite.Left, (int)height), _sprite.NinePatchRects[3], color);

                var diff = (_sprite.Left + _sprite.Right) - width;
                diff = _sprite.Left - diff;

                var rightRect = _sprite.NinePatchRects[4];
                var copyRightRect = new Rectangle(rightRect.X, rightRect.Y, (int)diff, rightRect.Height);

                batcher.Draw(_sprite, new Rectangle((int)x + _sprite.Left, (int)y, (int)diff, (int)height), copyRightRect, color);
            }
            else
            {
                for (var i = 0; i < 9; i++)
                {
                    ...
                    if (_destRects[i].Width <= 0 || _destRects[i].Height <= 0 || _destRects[i].X < 0 || _destRects[i].Y < 0)
                        continue;
