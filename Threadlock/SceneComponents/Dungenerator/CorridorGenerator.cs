...
namespace Threadlock.SceneComponents.Dungenerator
{
    public static class CorridorGenerator
    {
        ...
        public static bool ConnectDoorways(DungeonDoorway startDoor, DungeonDoorway endDoor, WeightedGridGraph graph, List<DungeonRoomEntity> roomsToCheck, out List<Vector2> floorPositions)
        {
            ...
            while (!isPathValid)
            {
                ...
                foreach (var room in roomsToCheck)
                {
                    ...
                    if (room.OverlapsRoom(posDictionary.Keys.ToList(), out var roomOverlaps))
                    ...
                    if (overlappingPositions.Any())
                    {
                        ...
                        foreach (var overlap in overlappingPositions)
                        {
                            ...
                            {
                                var adjustedPos = (parentPos / 16).ToPoint();
        ...
        public static bool ConnectDoorways(DungeonDoorway startDoor, DungeonDoorway endDoor, WeightedGridGraph graph, List<DungeonRoomEntity> roomsToCheck, out List<SingleTile> tiles)
        {
            ...
            while (!isPathValid)
            {
                ...
                //validate floor and tile positions
                List<DungeonComposite> visitedComposites = new List<DungeonComposite>();
                foreach (var room in roomsToCheck)
                {
                    List<Vector2> overlappingPositions = new List<Vector2>();

                    if (!visitedComposites.Contains(room.ParentComposite))
                    {
                        visitedComposites.Add(room.ParentComposite);
                        var overlaps = tileDictionary.SelectMany(t => t.Value).Where(v => room.ParentComposite.SingleTileRenderers.Any(s => s.Entity.Position == v.Position));
                        overlappingPositions.AddRange(overlappingPositions);
                    }
                    if (room.OverlapsRoom(tileDictionary.SelectMany(t => t.Value).Select(v => v.Position).ToList(), out var roomOverlaps))
                        overlappingPositions.AddRange(roomOverlaps);
